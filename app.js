...
var App = {
  ...
  run: function(options, callback) {
    ...
    // Discord token
    App.Discord.token = process.env.DISCORD_TOKEN
     ...
     * Presence Event
     */
    App.Discord.client.on("presenceUpdate", function(
      statusBefore,
      statusAfter
    ) {
  ...
  setup: function() {
    ...
    App.log = function(data = null, options = null) {
      ...
      // Discord logging
      if (
        isset(App.channelProcessingisReady) &&
        App.logger.service.discord.enabled &&
        options.discord
      )
    ...
    App.instance.checksum = checksum.file(path.basename(__filename), function(
      err,
      checksum
    ) {
      if (err) App.log(err);
  ...
  handleMessage: function(client, message) {
    ...
    if (message.content === "help") {
      message.reply(
        "[" +
          App.instance.id +
          "] Available commands: ping, uptime, restart, debug"
      );
    ...
    if (channel.type == "voice") {
      ...
      if (channel.name.startsWith(App.DynamicChannelName.channelPrefix)) {
          ...
          member.guild.presences.forEach(function(Presence, userId) {
            if (member.user.id === userId) {
              if (isset(Presence.game)) {
                presence.stats[Presence.game.name] = isset(
                  presence.stats[Presence.game.name]
                )
                  ? presence.stats[Presence.game.name] + 1
                  : 1;
              } else {
                presence.stats[".none"] = isset(presence.stats[".none"])
                  ? presence.stats[".none"] + 1
                  : 1;
              }
            }
          });
        ...
        for (presence.currentName in presence.stats) {
          presence.currentProcentage = Math.round(
            (presence.stats[presence.currentName] / channelParticipants) * 100,
            1
          );
          if (
            presence.currentProcentage > presence.highestProcentage &&
            presence.currentName != ".none"
          ) {
        ...
        if (channelParticipants === 0) {
          suggestion.channelName =
            App.DynamicChannelName.defaultChannelNameEmpty;
        } else if (
          // If presence is meeting target procentage, handle 2 participants.
          presence.highestProcentage >=
            App.DynamicChannelName.minPresenceDominanceProcentage ||
          (presence.highestProcentage >=
            App.DynamicChannelName.minPresenceDominanceProcentage - 1 &&
            channelParticipants >= 2)
        ) {
          // Let's do some presence rewrite, if nessesary
          if (presence.currentName == "Tom Clancy's The Division 2")
            presence.currentName = "The Division 2";

          if (presence.currentName == "iexplore")
            presence.currentName = "Silly Goose!";
          if (presence.currentName == "Project Argo (Prototype)")
            presence.currentName = "Argo";
          if (presence.currentName == "Unity")
            presence.currentName = "Programming";

          // Default to none
          if (presence.currentName == "Custom Status")
            presence.currentName = ".none";
          if (presence.currentName == "wallpaper_engine")
            presence.currentName = ".none";
          if (presence.currentName == "Spotify") presence.currentName = ".none";

          // If we have none, let's grab the default channel name
          if (presence.currentName == ".none")
            presence.currentName = App.DynamicChannelName.defaultChannelName;
        ...
        suggestion.channelName =
          suggestion.channelName.length >
          App.DynamicChannelName.maxChannelNameLength
            ? suggestion.channelName.substring(
                0,
                App.DynamicChannelName.maxChannelNameLength
              ) + "..."
            : suggestion.channelName;
        ...
        while (nameGenerator.run) {
          ...
          if (nameGenerator.count > 1) {
            suggestion.channelNumber =
              nameGenerator.count < 10
                ? "0" + nameGenerator.count
                : nameGenerator.count;
          }
          ...
          suggestion.channelNumber =
            "(" +
            App.DynamicChannelName.defaultChannelName +
            " " +
            presence.totalChannels +
            ")";
          ...
          if (nameGenerator.count > App.DynamicChannelName.maxChannelSpawn)
            break;
        }
        ...
        if (
          App.DynamicChannelName.enabled &&
          channel.name != suggestion.final
        ) {
          App.log("DCN: Changing channel name to '" + suggestion.final + "'");
          App.log(
            "DCN: Changing channel name from '" +
              channel.name +
              "' to '" +
              suggestion.final +
              "'.\n",
          ...
          App.log(
            "DCN: Channel name is already per suggestion: " +
              suggestion.final +
              "'"
          );
        } else {
          App.log(
            "DCN: Suggesting changing channel name to '" +
              suggestion.final +
              "'"
          );
        }
